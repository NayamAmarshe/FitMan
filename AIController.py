...
#         frame = cv2.flip(frame,2)
        ...
        try:
            ...
            left_elbow_angle = calculate_elbowangle(lefthip,leftshoulder,leftelbow)
            right_elbow_angle = calculate_elbowangle(righthip,rightshoulder,rightelbow)
            
            #Calculate Wrist Angle
            left_wrist_angle = calculate_wristangle(leftshoulder,leftelbow,leftwrist)
            right_wrist_angle = calculate_wristangle(rightshoulder,rightelbow,rightwrist)
            
            #visualize
            #left elbow
            cv2.putText(image,str(left_elbow_angle),tuple(np.multiply(leftelbow,[640,480]).astype(int)),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),2,cv2.LINE_AA)
            #right elbow
            cv2.putText(image,str(right_elbow_angle),tuple(np.multiply(rightelbow,[640,480]).astype(int)),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),2,cv2.LINE_AA)
            #left wrist
            cv2.putText(image,str(left_wrist_angle),tuple(np.multiply(leftwrist,[640,480]).astype(int)),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),2,cv2.LINE_AA)
            #right wrist
            cv2.putText(image,str(right_wrist_angle),tuple(np.multiply(rightwrist,[640,480]).astype(int)),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),2,cv2.LINE_AA)
